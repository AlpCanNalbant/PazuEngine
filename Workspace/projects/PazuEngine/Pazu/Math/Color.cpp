...
namespace Pazu
{
	...
	Color::Color(const Vector3 &clr)
			: XMFLOAT4(clr.x, clr.y, clr.z, 1.f) {}

	Color::Color(const Vector4 &clr)
			: XMFLOAT4(clr.x, clr.y, clr.z, clr.w) {}

	Color::Color(_In_reads_(4) const float *pArray)
			: XMFLOAT4(pArray) {}

	Color::Color(DirectX::FXMVECTOR V)
	{
		XMStoreFloat4(this, V);
	}

	Color::Color(const XMFLOAT4 &c)
	{
		this->x = c.x;
		this->y = c.y;
		this->z = c.z;
	...
	Color::Color(const DirectX::XMVECTORF32 &F)
	{
		this->x = F.f[0];
		this->y = F.f[1];
		this->z = F.f[2];
	...
	Color &Color::operator=(const DirectX::XMVECTORF32 &F)
	{
		x = F.f[0];
		y = F.f[1];
		z = F.f[2];
...
namespace std
{
	bool less<Pazu::Color>::operator()(const Pazu::Color &C1, const Pazu::Color &C2) const
	{
