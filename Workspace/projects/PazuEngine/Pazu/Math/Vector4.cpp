...
namespace Pazu
{
	...
	Vector4::Vector4(const XMFLOAT4 &V)
	{
		this->x = V.x;
		this->y = V.y;
		this->z = V.z;
	...
	Vector4::Vector4(const DirectX::XMVECTORF32 &F)
	{
		this->x = F.f[0];
		this->y = F.f[1];
		this->z = F.f[2];
	...
	Vector4 &Vector4::operator=(const DirectX::XMVECTORF32 &F)
	{
		x = F.f[0];
		y = F.f[1];
		z = F.f[2];
	...
	_Use_decl_annotations_ void Vector4::Transform(const Vector4 *varray, size_t count, const Matrix &m, Vector4 *resultArray)
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
...
namespace std
{
	bool less<Pazu::Vector4>::operator()(const Pazu::Vector4 &V1, const Pazu::Vector4 &V2) const
	{
