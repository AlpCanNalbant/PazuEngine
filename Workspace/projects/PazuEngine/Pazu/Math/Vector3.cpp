...
namespace Pazu
{
	...
	Vector3::Vector3(const DirectX::XMVECTORF32 &F)
	{
		this->x = F.f[0];
		this->y = F.f[1];
		this->z = F.f[2];
	...
	Vector3 &Vector3::operator=(const DirectX::XMVECTORF32 &F)
	{
		x = F.f[0];
		y = F.f[1];
		z = F.f[2];
	...
	_Use_decl_annotations_ void Vector3::Transform(const Vector3 *varray, size_t count, const Matrix &m, Vector3 *resultArray)
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
	...
	_Use_decl_annotations_ void Vector3::Transform(const Vector3 *varray, size_t count, const Matrix &m, Vector4 *resultArray)
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
	...
	_Use_decl_annotations_ void Vector3::TransformNormal(const Vector3 *varray, size_t count, const Matrix &m, Vector3 *resultArray)
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
...
namespace std
{
	bool less<Pazu::Vector3>::operator()(const Pazu::Vector3 &V1, const Pazu::Vector3 &V2) const
	{
