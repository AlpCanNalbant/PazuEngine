...
namespace Pazu::Math
{
	...
	Vector2::Vector2(const DirectX::XMVECTORF32 &F) noexcept
	{
		this->x = F.f[0];
		this->y = F.f[1];
	}
	...
	Vector2 &Vector2::operator=(const DirectX::XMVECTORF32 &F) noexcept
	{
		x = F.f[0];
		y = F.f[1];
		return *this;
	...
	_Use_decl_annotations_ void Vector2::Transform(const Vector2 *varray, size_t count, const Matrix &m, Vector2 *resultArray) noexcept
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
	...
	_Use_decl_annotations_ void Vector2::Transform(const Vector2 *varray, size_t count, const Matrix &m, Vector4 *resultArray) noexcept
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
	...
	_Use_decl_annotations_ void Vector2::TransformNormal(const Vector2 *varray, size_t count, const Matrix &m, Vector2 *resultArray) noexcept
	{
		using namespace DirectX;
		XMMATRIX M = XMLoadFloat4x4(&m);
...
namespace std
{
	bool less<Pazu::Math::Vector2>::operator()(const Pazu::Math::Vector2 &V1, const Pazu::Math::Vector2 &V2) const noexcept
	{
