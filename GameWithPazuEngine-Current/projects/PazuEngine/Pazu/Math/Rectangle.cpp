...
namespace Pazu
{
	...
	Vector2 Rectangle::Location() const
	{
	...
	bool Rectangle::Contains(long ix, long iy) const
	{
	...
	void Rectangle::Inflate(long horizAmount, long vertAmount)
	{
		x -= horizAmount;
		y -= vertAmount;
		width += horizAmount;
	...
	void Rectangle::Offset(long ox, long oy)
	{
		x += ox;
		y += oy;
	}

	Rectangle Rectangle::Intersect(const Rectangle &ra, const Rectangle &rb)
	{
	...
	Rectangle Rectangle::Union(const Rectangle &ra, const Rectangle &rb)
	{
		long righta = ra.x + ra.width;
		long rightb = rb.x + rb.width;
...
namespace std
{
	bool less<Pazu::Rectangle>::operator()(const Pazu::Rectangle &r1, const Pazu::Rectangle &r2) const
	{
