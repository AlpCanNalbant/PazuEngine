...
namespace Pazu
{
	...
	bool Ray::Intersects(const DirectX::BoundingSphere &sphere, _Out_ float &Dist) const
	{
		return sphere.Intersects(position, direction, Dist);
	}

	bool Ray::Intersects(const DirectX::BoundingBox &box, _Out_ float &Dist) const
	{
...
namespace std
{
	bool less<Pazu::Ray>::operator()(const Pazu::Ray &R1, const Pazu::Ray &R2) const
	{
